...
| 公开博客站点 | 骨架完成 | 已建立 Astro 应用，当前为最小首页入口 |
| 发布后台 | 骨架完成 | 已建立 React + Vite 界面，支持本地演示编辑、草稿和发布状态 |
| 发布 API | 已增强 | FastAPI + SQLite 已实现文章增删改查、详情、搜索、草稿/发布切换和后台汇总 |
...
1. 完成公开站点基础页面和 Markdown 内容目录。
2. 将后台编辑页面接入 FastAPI，实现文章列表、创建、编辑、删除和状态更新。
3. 增加单作者登录、会话校验和基础错误反馈。
4. 将已发布文章同步为公开站点可构建的 Markdown 内容。
5. 实现发布流程：校验内容、创建 Git commit、推送 GitHub。
6. 配置 GitHub Actions，完成构建、部署和失败反馈。
7. 接入搜索、RSS、sitemap、Open Graph 和基础 SEO。
8. 使用真实文章完成桌面端、移动端和发布流程验收。
